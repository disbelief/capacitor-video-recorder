...
export class VideoRecorderWeb extends WebPlugin implements VideoRecorderPlugin {
	...
	private _updateCameraView(config: FrameConfig): void {
		if (this.videoElement !== null) {
			...
			this.videoElement.style.boxShadow = `0 0 ${config.dropShadow?.radius || 0}px 0 rgba(${
				config.dropShadow?.color
			}, ${config.dropShadow?.opacity})`
		}
	}

	private _decideMimeType(): string | undefined {
		const isQueriable = Object.prototype.hasOwnProperty.call(MediaRecorder, 'isTypeSupported')
		const allowIfSupported = (codec: string) =>
			isQueriable && MediaRecorder.isTypeSupported(codec) ? codec : false
		...
		const supported = [
			...
			undefined,
		].filter((codec): codec is string | undefined => codec !== false)
